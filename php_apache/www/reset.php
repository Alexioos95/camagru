			...
			else
			{
				...
				else
				{
					...
					$emailBody = "
						<html>
							<head>
								<title>camagru - Your reset request</title>
							</head>
							<body>
								<p>Click the link below to reset your password to camagru:</p>
								<p><a href='$emailLink'>$emailLink</a></p>
							</body>
						</html>
					";
					$mailReq = $pdo->prepare("INSERT INTO mailQueue (email, subject, body) VALUES (?, ?, ?)");
					$mailReq->execute([$email, "camagru - Your reset request", $emailBody]);
